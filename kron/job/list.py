...
def list_jobs(server: JobServer):
    jobs = server.list()
    data = [['Id', 'Alias', 'Name/Description', 'Schedule', 'Cron Type', 'Last Status', 'Last run', 'Next run', 'Time Zone', 'Paused']]
    for job in jobs:
        data.append(
            [job['id'], util.format_none(job['alias'] if 'alias' in job else ''), job['name'], job['scheduleText'],
             job['cronType'], util.format_status(job['lastStatus']), job['lastRun'], job['nextRun'], job['timeZone'], job['paused']])
